...
AND is true if both operands are equal or both operands are not equal.
...
Most statements in the C programming language is an expression. A combination of operators and operands is called an expression.
...
* To declare a pointer, the indirection operator will always be at the left side of the assign operator.
...
**Today's Progress:** Structures - Initialization
 
**Thoughts:** Similar to arrays, you initialize structure in curly braces separated by commas `{a, b, c, etc...}`. The value initialized must be constant expressions.
 
One more thing, values must be initialized with a custom variable that come from the struct itself.. I will dive into this more deeply in the upcoming days, but just to learn how to initialize I will use this today.
 
It is alright if you initialize less values than what the structure take, it will still work. you can also put them in any order independent on how they are stored in the structure.
 
Another way to initialize values is by using a dot notation. The concept of object oriented programming, this mean C have object oriented capabilities. 
...
**Today's Progress:** Structures - Definition
 
**Thoughts:** Just to point out, A structure can be put on the global level so any function can use it. 
 
There is a flexibility in defining a structure. A definition of a structure is basically a combination of declaration and initialization to prepare the structure for the program.
 
You can instantly declare a variable of the type struct right after the struct by including the name of it before the end of the struct definition and after the braces. It is still considered a definition even if it has some extra lines. You can also initialize the struct values at the same time.
...
**Today's Progress:** Structures - Declaring a custom struct variable
 
**Thoughts:** At the declaration stage of a structure, you are actually creating a new data type that is named as the structure name. and it is funny, because structures can have different data types, as many as you like.
 
You can create custom variables of the structure type so that they can use the elements of the structure. The variables will have an allocated memory space automatically assigned to them.
...
**Today's Progress:** File I/O - Reading Strings
 
**Thoughts:** There are three types of operations in reading from files.
 
| Function | Description             |
|----------|-------------------------|
| fgetc()  | Reading a character     |
| fgets()  | Reading a string        |
| fgetf()  | Reading a formatted text |
...
* `fgets(x, y, z)`
    * x = character string
    * y = number of characters to read
    * z = file pointer
 
* `fgetf(x, y)`
    * x = file pointer
    * y = format. custom format
